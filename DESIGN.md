...
> **Adaptation for the diary app:** the Movies & TV implementation uses this treatment more broadly than "sparingly" — every explicit add-action (add tag, add date, add thought, add episode) is filled Ink rather than ghost-outlined. Unlike this doc's reference site, those aren't one occasional conversion moment; they're the primary way the user adds real data on every form, so each earns the same affordance as the top-level Add Movie/Save actions instead of receding into a quiet ghost button.
